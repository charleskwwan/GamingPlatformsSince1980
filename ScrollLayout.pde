...
public class ScrollLayout extends ViewPort {
  ...
  public ScrollLayout(float x, float y, float w, float h, Frame[] frames) {
    super(x, y, w, h);
  ...
  private float bottomPadding(int i) {
    return getH() * (i == 0 ? .4 : .05); // .4 for intro for chart
  }
  ...
  private void drawFrameChart(int i) {
    ...
    if (this.chart == null) {
      ...
      this.chart.draw(getX() + (getW() + xoffset - w) / 2, getY() + getH() * .1, w, w);
    }
  }
